...
class RSession(object):
    ...
    def call_function(self, func_name, *args, enable_output_conversion=True, convert_scalars=True, **kwargs):
        ...
        with self.input_converter():
            try:
                ...
                raise AttributeError(f'R function "{func_name}" could not be found in R session')

    def get_attribute(self, attr_name, enable_output_conversion=True, convert_scalars=True):
        ...
        try:
            ...
            raise AttributeError(f'R attribute "{attr_name}" could not be found in R session')

    def run_r_script_string(self, script_string, enable_output_conversion=True, convert_scalars=True):
    ...
    @staticmethod
    def convert_r_response_to_python(response, convert_scalars=True, to_list=False, native_to_json=False):
    ...
    def input_converter():
        ...
        return localconverter(default_converter + pandas2ri.converter + numpy2ri.converter)


def run_single_r_script_function(script_path, function_name, *args, enable_output_conversion=True,
                                 convert_scalars=True, **kwargs):
